...
public class FileSystemScanner {
	...
	public FileSystemScanner(final Activity activity, BooksAdapter booksAdapter) {
	...
	class ScanTask extends AsyncTask<String, String, Void> {
		...
		protected void onPostExecute(final Void v) {
			...
			if (BooksAdapter.SEARCH_PAUSED.getPreferenceValue(PreferenceManager.getDefaultSharedPreferences(activity), false)) {
				String q = LibSettings.current().searchBookQuery;
				if (q != null && q.length() > 0)
					booksAdapter.startSearch(q);
			}
		}
