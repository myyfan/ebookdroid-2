...
public class RecentActivityController extends AbstractActivityController<RecentActivity> implements IBrowserActivity,
	...
	public void beforeCreate(final RecentActivity activity) {
		...
		bookshelfAdapter = new BooksAdapter(this, recentAdapter, getActivity());
	...
	public void showDocument(final Uri uri, final Bookmark b) {
		showDocument(uri, b, null);
	}
	@Override
	public void showDocument(Uri uri, Bookmark b, BookShelfAdapter adapter) {
		...
		if (adapter != null) {
			int pos = bookshelfAdapter.getShelfPosition(adapter);
			String searchTerm = LibSettings.current().searchBookQuery;
			if (pos == BooksAdapter.SEARCH_INDEX && searchTerm.length() > 0) {
				// On search results, let doc know about search term...
				intent.putExtra("searchTerm", LibSettings.current().searchBookQuery);
			}
		}
		...
		try {
			...
			if (diff.isUseBookcaseChanged()) {
				...
				if (newSettings.useBookcase) {
					...
					getManagedComponent().showLibrary(bookshelfAdapter, recentAdapter);
				}
				return;
			}
