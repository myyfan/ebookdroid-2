...
public class RecentActivity extends AbstractActionActivity<RecentActivity, RecentActivityController> {
	...
	void showLibrary(final BooksAdapter libraryAdapter, final RecentAdapter recentAdapter) {
		final ViewFlipper vf = getViewflipper();
		vf.removeAllViews();
		bookcaseView = (BookcaseView) LayoutInflater.from(this).inflate(R.layout.list_view, vf, false);
		bookcaseView.init(libraryAdapter, recentAdapter);
		vf.addView(bookcaseView, 0);
	}
