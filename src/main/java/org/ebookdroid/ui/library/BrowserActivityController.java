...
public class BrowserActivityController extends AbstractActivityController<BrowserActivity> implements IBrowserActivity {
	...
	public boolean onKeyDown(final int keyCode, final KeyEvent event) {
		if (keyCode == KeyEvent.KEYCODE_BACK && event.getRepeatCount() == 0) {
			...
			if (dir.equals(BaseDroidApp.HOME_DIR)) {
				return false;
			}
			if (parent != null) {
	...
	public void goHome(final ActionEx action) {
		if (BaseDroidApp.HOME_DIR.exists()) {
			setCurrentDir(BaseDroidApp.HOME_DIR);
	...
	@Override
	public void showDocument(final Uri uri, final Bookmark b, BookShelfAdapter adapter) {
		final BrowserActivity activity = getManagedComponent();
