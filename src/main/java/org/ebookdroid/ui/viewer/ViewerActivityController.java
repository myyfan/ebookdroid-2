...
public class ViewerActivityController extends AbstractActivityController<ViewerActivity> implements
	...
	public void beforeCreate(final ViewerActivity activity) {
		...
		if (intent == null) {
			showErrorDlg(R.string.msg_bad_intent, "NULL");
			return;
		}
	...
	public void afterCreate(final ViewerActivity activity, final boolean recreated) {
		...
		getActivity().getSearchControls().setShowTotals(documentModel.decodeService.
				isFeatureSupported(CodecFeatures.FEATURE_DOCUMENT_TEXT_SEARCH));
	}
		...
		final Runnable r = new Runnable() {
			...
			public void run() {
				try {
					...
					activity.setSupportProgressBarIndeterminateVisibility(currentlyDecoding > 0);
					activity.getWindow().setFeatureInt(Window.FEATURE_INDETERMINATE_PROGRESS,
							currentlyDecoding == 0 ? 10000 : currentlyDecoding);
//					activity.supportRequestWindowFeature(Window.FEATURE_INDETERMINATE_PROGRESS);
				} catch (final Throwable e) {
					LCTX.e("decodingProgressChanged failed", e);
				}
			}
		};
	 ...
	 * {@inheritDoc}
	 *
	 * @see org.ebookdroid.ui.viewer.IActivityController#jumpToPage(int, float, float, boolean)
	 */
	@Override
	...
	public final void doSearch(final ActionEx action) {
		...
		executor.execute(new SearchTask(),
				newPattern, oldPattern, (String) action.getParameter("forward"));
	}
	...
	public void showBookmarkDialog(final ActionEx action) {
		...
		if (input != null) {
			input.setText(activity.getString(R.string.text_page) + " " + (page + offset));
			input.selectAll();
			final ActionDialogBuilder builder = new ActionDialogBuilder(activity, this);
	...
	final class SearchTask extends AsyncTask<String, String, RectF> implements SearchModel.ProgressCallback,
		...
		@Override
		public void setTotalFound(final int total) {
			ViewerActivity.i().runOnUiThread(new Runnable() {
				@Override
				public void run() {
					getActivity().getSearchControls().setTotal(total);
				}
			});
		}
		...
		public void setCurrentItem(final int currentItem) {
			ViewerActivity.i().runOnUiThread(new Runnable() {
				@Override
				public void run() {
					getActivity().getSearchControls().setCurrent(currentItem);
				}
			});
		}
		...
		protected void onPostExecute(final RectF result) {
			if (progressDialog != null) {
				try {
					...
					LCTX.e("Progress Dialog dismiss failure");
				}
			}
			if (result != null) {
				...
				boolean manageSearch = getDocumentModel().decodeService.
						isFeatureSupported(CodecFeatures.FEATURE_DOCUMENT_MANAGE_SEARCH);
				if (!manageSearch)  // Do not need this if managed.
