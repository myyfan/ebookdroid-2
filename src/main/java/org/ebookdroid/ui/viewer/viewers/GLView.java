...
public final class GLView extends GLRootView implements IView, SurfaceHolder.Callback {
	 ...
	 */
	@Override
	public void onScrollChange(final int curX, final int curY, final int oldX, final int oldY) {
	...
	public boolean onTouchEvent(final MotionEvent ev) {
		...
		try {
			checkFullScreenMode();
			return base.getDocumentController().onTouchEvent(ev) || super.onTouchEvent(ev);
		} finally {
			mRenderLock.unlock();
		}
	}
	...
	protected final void onLayout(final boolean layoutChanged, final int left, final int top, final int right,
		...
		final Rect oldLayout = layout.getAndSet(layoutRect);
