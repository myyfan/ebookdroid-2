...
import org.ebookdroid.common.bitmaps.*;
...
public class DecodeServiceBase implements DecodeService {
	...
	void performDecode(final DecodeTask task) {
		...
		try {
			...
			r = getScaledSize(task.node, task.viewState.zoom, croppedPageBounds);
			...
			final IBitmapCore bitmap = vuPage.renderBitmap(task.viewState, r.width(), r.height(),
					actualSliceBounds);
			...
			finishDecoding(task, vuPage, bitmap, croppedPageBounds);
	...
	protected RectF calculateRootCropping(final DecodeTask task, final PageTreeNode root, final CodecPage vuPage) {
		...
		final IBitmapCore rootBitmap = vuPage.renderBitmap(task.viewState, PageCropper.BMP_SIZE,
	...
	Rect getScaledSize(final PageTreeNode node, final float zoom, final RectF croppedPageBounds) {
	...
	void finishDecoding(final DecodeTask currentDecodeTask, final CodecPage page, final IBitmapCore bitmap,
						final RectF croppedPageBounds) {
		stopDecoding(currentDecodeTask.node, "complete");
		updateImage(currentDecodeTask, page, bitmap, croppedPageBounds);
	}

	void abortDecoding(final DecodeTask currentDecodeTask, final CodecPage page, final IBitmapCore bitmap) {
	...
	void updateImage(final DecodeTask currentDecodeTask, final CodecPage page, final IBitmapCore bitmap,
					 final RectF croppedPageBounds) {
	...
	class Executor implements Runnable {
		...
		void start() {
			...
			for (Thread t: threads) {
				t.setPriority(decodingThreadPriority);
				t.start();
			}
		}
	...
	class DecodeTask extends Task {
		...
		public String toString() {
			return "DecodeTask" +
				...
				"id=" +id + ", " +
				"target=" + node + ", " +
				"width=" + viewState.viewRect.width() + ", " +
				"zoom=" + viewState.zoom + "]";
		}
	}
	...
	public IBitmapRef createThumbnail(final boolean useEmbeddedIfAvailable, int width, int height, final int pageNo,
		...
		if (thumbnail != null) {
			...
			return retB==null?null:retB.toBitmap();
		}
	}

	@Override
	public IBitmapCore createPageThumbnail(final int width, final int height, final int pageNo, final RectF region) {
		if (document == null) {
			return null;
		}
		final CodecPage page = getPage(pageNo);
