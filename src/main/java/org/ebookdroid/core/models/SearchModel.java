...
public class SearchModel {
	...
	public RectF moveToNext(final ProgressCallback callback) {
		...
		boolean manageSearch = base.getDocumentModel().decodeService.
				isFeatureSupported(CodecFeatures.FEATURE_DOCUMENT_MANAGE_SEARCH);
		if (manageSearch) {
			base.getDocumentModel().decodeService.searchNext(true);
			currentMatchIndex++;
			if (currentMatchIndex >= totalFound) currentMatchIndex = 0;
			callback.setCurrentItem(currentMatchIndex + 1);
		} else {
	...
	public RectF moveToPrev(final ProgressCallback callback) {
		...
		boolean manageSearch = base.getDocumentModel().decodeService.
				isFeatureSupported(CodecFeatures.FEATURE_DOCUMENT_MANAGE_SEARCH);
		if (manageSearch) {
			base.getDocumentModel().decodeService.searchNext(false);
			currentMatchIndex--;
			if (currentMatchIndex < 0) currentMatchIndex = totalFound > 0?totalFound - 1:0;
			callback.setCurrentItem(currentMatchIndex + 1);
		} else {
			if (firstVisiblePage <= currentPage.index.viewIndex && currentPage.index.viewIndex <= lastVisiblePage) {
				currentMatchIndex--;
	...
	private RectF searchFirstFrom(final int pageIndex, final ProgressCallback callback) {
		...
		if (index < -1) index = -1;  // XXX Why does this happen?
		while (!(callback != null && callback.isCancelled()) && ++index < pageCount) {
			...
			if (callback != null) {
				...
				if (mm != null) {
					callback.setTotalFound(totalFound);
					callback.setCurrentItem(mm.size() > 0?1:0);
				}
			}
			if (LengthUtils.isNotEmpty(mm)) {
				currentPage = p;
				currentMatchIndex = 0;
				if (mm != null && mm.size() > 0) return mm.get(currentMatchIndex);
			}
		}
		if (LCTX.isDebugEnabled()) {
	...
	private RectF searchLastFrom(final int pageIndex, final ProgressCallback callback) {
		...
		int index = pageIndex + 1;
		while (!(callback != null && callback.isCancelled()) && 0 <= --index) {
			final Page p = base.getDocumentModel().getPageObject(index);
			if (callback != null) {
				callback.searchStarted(index);
			}
			...
			if (callback != null) {
				...
				if (mm != null) {
					callback.setTotalFound(totalFound);
					callback.setCurrentItem(mm.size() > 0?mm.size() - 1:0);
				}
			}
			if (LengthUtils.isNotEmpty(mm)) {
				currentPage = p;
				currentMatchIndex = mm != null?mm.size() - 1:0;
				if (mm != null && mm.size() > 0) return mm.get(currentMatchIndex);
			}
		}
		return null;
