...
import org.ebookdroid.common.settings.listeners.*;
...
public class SettingsManager {
	...
	public static Map<String, BookSettings> getSearchFound() {
		lock.writeLock().lock();
		try {
			return db.getSearchFound();
		} finally {
			lock.writeLock().unlock();
		}
	}

	public static Map<String, BookSettings> getSearchNotFound() {
		lock.writeLock().lock();
		try {
			return db.getSearchNotFound();
		} finally {
			lock.writeLock().unlock();
		}
	}

	public static boolean storeSearch(final String book, final boolean found) {
		lock.writeLock().lock();
		try {
			return db.storeSearch(book, found);
		} finally {
			lock.writeLock().unlock();
		}
	}
