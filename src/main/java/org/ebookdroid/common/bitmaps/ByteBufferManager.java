...
public class ByteBufferManager {
		...
		try {
			...
			while (!releasing.isEmpty()) {
				...
				if (ref instanceof ByteBufferBitmap) {
					...
					final IBitmapCore[] bitmaps = bmp.clear();
					if (bitmaps != null) {
						for (final IBitmapCore b : bitmaps) {
							if (b instanceof ByteBufferBitmap) releaseImpl((ByteBufferBitmap)b);
							else b.release();
							count++;
						}
					}
	...
	public static void release(final IBitmapCore[] refs) {
		if (refs != null) {
			if (refs instanceof ByteBufferBitmap[]) {
				if (LCTX.isDebugEnabled()) {
					LCTX.d("Adding " + refs.length + " refs to release queue");
				}
				releasing.add(refs);
			} else {
				for (IBitmapCore bm : refs) {
					bm.release();
				}
			}
		}
