...
//	protected static final ThreadLocal<ByteBufferBitmap> threadSlices = new ThreadLocal<ByteBufferBitmap>();
	...
	private IBitmapCore[] bitmaps;

	private ByteBufferTexture[] textures;

	public GLBitmaps(final String nodeId, final IBitmapCore orig, final PagePaint paint) {
		this.nodeId = nodeId;
		...
		if (orig instanceof ByteBufferBitmap) {
			this.bitmaps = ByteBufferManager.getParts(partSize, rows, columns);
		} else {
			this.bitmaps = new IBitmapCore[] { orig };
		}
			...
			for (int col = 0; col < columns; col++, left += partSize) {
				...
				try {
					if (row == rows - 1 || col == columns - 1) {
						final int right = Math.min(left + partSize, orig.getWidth());
						final int bottom = Math.min(top + partSize, orig.getHeight());
	...
	IBitmapCore[] clear() {
		...
		try {
			if (textures != null) {
				for (ByteBufferTexture t: textures) {
					t.recycle();
				}
				textures = null;
			}

			final IBitmapCore[] ref = this.bitmaps;
			this.bitmaps = null;
			return ref;
		} finally {
			lock.writeLock().unlock();
