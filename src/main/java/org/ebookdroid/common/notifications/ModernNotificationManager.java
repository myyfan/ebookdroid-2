...
class ModernNotificationManager extends AbstractNotificationManager {
	...
	public int notify(final CharSequence title, final CharSequence message, final Intent intent) {
		return notify(null, title, message, intent);
	}

	@Override
	public int notify(final Integer id, final CharSequence title, final CharSequence message, final Intent intent) {
		...
		int newId = id==null?SEQ.getAndIncrement():id;
		getManager().notify(newId, notification);
